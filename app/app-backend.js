// NOTE: This is if we had backend
const baseurl = `/api/weather`;
function fetchWeatherData(query, days) {
  const url = `${baseurl}?q=${query}&days=${days}`;
...
function getLocationWeather(len) {
  ...
  let length = len;
  if (city) {
    return fetchWeatherData(city, length)
      .then((data) => {
...
function getCurrentLocationWeather(lat, lon) {
  ...
  return fetchWeatherData(query, valueFromDayLength)
    .then((data) => {
      const today = new Date();
      const options = { month: "short", day: "numeric" };
      const formattedDate = today.toLocaleDateString("en-US", options);
